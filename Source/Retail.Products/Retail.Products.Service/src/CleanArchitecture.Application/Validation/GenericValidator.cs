...
namespace Retail.Api.Products.src.CleanArchitecture.Application.Validation
{
    ...
    public static class GenericValidator
    {
        ...
        /// <summary>
        /// Validates if a required identifier value is valid (must be greater than zero, cannot be zero for new entities).
        /// </summary>
        /// <typeparam name="T">Type of the identifier.</typeparam>
        /// <param name="identifier">The identifier value to validate.</param>
        /// <param name="validatorName">Name of Validator.</param>
        ...
        /// <param name="validationData">The result of the validation.</param>
        /// <returns>True, if the identifier is invalid (zero or negative). False otherwise.</returns>
        public static bool HasInvalidRequiredIdentifier<T>(T identifier, string validatorName, string fieldName, out ValidationData validationData)
        {
            if ((identifier is int id && id <= 0) ||
                (identifier is long longId && longId <= 0) ||
                (identifier is string str && string.IsNullOrWhiteSpace(str)) ||
                (identifier is Guid guid && guid == Guid.Empty))
            {
                validationData = new ValidationData(
                    validatorName: validatorName,
                    failureReason: $"The {fieldName} field must have a valid value greater than zero.");
                return true;
            }
        ...
        /// <summary>
        /// Validates if a string exceeds a maximum length.
        /// </summary>
        /// <param name="value">The string value to validate.</param>
        ...
        /// <param name="validationData">The validation data to return.</param>
        /// <param name="failureSeverity">The severity level of the validation if it fails.</param>
        /// <returns>True if the string exceeds the max length, false otherwise.</returns>
        public static bool ExceedsMaxLength(string value, int maxLength, string validatorName, string fieldName, out ValidationData validationData, FailureSeverity failureSeverity = FailureSeverity.Error)
        {
            if (value != null && value.Length > maxLength)
            {
                validationData = new ValidationData(
                    validatorName: validatorName,
                    failureReason: $"The {fieldName} exceeds the maximum length of {maxLength}.",
                    failureSeverity: failureSeverity);
                return true;
            }
        ...
        /// <summary>
        /// Validates if a numeric value is less than zero.
        /// </summary>
        ...
        public static bool IsLessThanZero(double value, string validatorName, string fieldName, out ValidationData validationData, FailureSeverity failureSeverity = FailureSeverity.Error)
        {
            if (value < 0)
            {
                validationData = new ValidationData(
                    validatorName: validatorName,
                    failureReason: $"The {fieldName} field must be greater than or equal to zero.",
                    failureSeverity: failureSeverity);
                return true;
            }
        ...
        /// <summary>
        /// Validates if an integer value is less than zero.
        /// </summary>
        /// <param name="value">The value to validate.</param>
        /// <param name="validatorName">Name of Validator.</param>
        /// <param name="fieldName">The name of the field being validated.</param>
        ...
        /// <param name="failureSeverity">The severity level of the validation if it fails.</param>
        /// <returns>True if the value is less than zero, false otherwise.</returns>
        public static bool IsLessThanZero(int value, string validatorName, string fieldName, out ValidationData validationData, FailureSeverity failureSeverity = FailureSeverity.Error)
        {
            if (value < 0)
            {
                validationData = new ValidationData(
